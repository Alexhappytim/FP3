...
module InterpolationTests =
    ...
    let ``Lagrange interpolation for two points equals linear``() =
        ...
        Assert.True(abs (linear - lagrange) < epsilon,
            sprintf "Expected %f but got %f" linear lagrange)
    ...
    let ``Newton with parabola points y = x^2``() =
        ...
        Assert.True(abs (result - 2.25) < 0.1,
            sprintf "Expected ~2.25 but got %f" result)
